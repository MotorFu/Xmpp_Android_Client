...
public class SmackImpl {
    ...
    public SmackImpl(XmppService service) {
        ...
        boolean smackdebug = PreferenceUtils.getPrefBoolean(service, PreferenceConstants.SMACKDEBUG, false);// 是否需要smack debug
     ...
     * 将消息添加到数据库
     *
     * @param direction       是否为收到的消息INCOMING为收到，OUTGOING为自己发出
     * @param JID             此消息对应的jid
     * @param message         消息内容
     * @param delivery_status 消息状态 DS_NEW为新消息，DS_SENT_OR_READ为自己发出或者已读的消息
     * @param ts              消息时间戳
     * @param packetID        服务器为了区分每一条消息生成的消息包的id
     */
     ...
     * 改变消息状态
     *
     * @param packetID   消息的id
     * @param new_status 新状态类型
     */
